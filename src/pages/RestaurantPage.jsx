...
export default function RestaurantPage() {
  ...
  async function loadUserData() {
    const [vRes, avRes] = await Promise.all([
  ...
  async function handleShare() {
    const url = `https://littlefoodies.app/restaurant/${id}`
    const message = `Check out this restaurant on Little Foodies! 🍽️ ${restaurant?.name || ''} — ${url}`
    try {
      if (navigator.share) {
        // Opens the phone's native share sheet (Messages, etc.)
        await navigator.share({ title: 'Little Foodies', text: message })
      } else if (navigator.clipboard) {
        // Desktop browsers without a share sheet: copy so they can paste into a text.
        await navigator.clipboard.writeText(message)
        showToast('Link copied — paste it into a text! 📋')
      } else {
        showToast('Sharing isn’t supported on this device', false)
      }
    } catch (e) {
      // The user closed the share sheet, or it was cancelled — nothing to do.
    }
  }
